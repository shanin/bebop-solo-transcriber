...
def main(args):
    ...
    filosax_train_dt = FilosaxDataset(data_dir=args.filosax_dir, split = 'train', source = 'double_time')
    filosax_train_dt_segments = SegmentDataset(filosax_train_dt, num_consecutive_bars = 8, random_transposition = args.transpose_augmentation)
    #filosax_train_dt_loader = DataLoader(filosax_train_dt_segments, batch_size=512, shuffle=True, num_workers=0, pin_memory=True)
    ...
    dataset_train = ConcatDataset([filosax_train_segments, filosax_train_dt_segments])
    len_orig = len(filosax_train_segments)
    len_dt = len(filosax_train_dt_segments)

    weight_orig = 0.5
    weight_dt = 0.5

    weights = [weight_orig] * len_orig + [weight_dt] * len_dt
